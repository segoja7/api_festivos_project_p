...
resource "aws_codebuild_project" "api_build_project" {
  name          = "${local.workspace.codebuild_project_name}-api"
  description   = "API build project for ${local.workspace.pipeline_name}"
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "terragrunt/resources/ci-cd/codepipeline/buildspecs/buildspec-api-build.yml"
  }
...
resource "aws_codebuild_project" "frontend_build_project" {
  name          = "${local.workspace.codebuild_project_name}-frontend"
  description   = "Frontend build project for ${local.workspace.pipeline_name}"
  service_role  = var.codebuild_role_arn
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "terragrunt/resources/ci-cd/codepipeline/buildspecs/buildspec-frontend-build.yml"
  }
